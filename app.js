...
(() => {
  ...
  const iconSrc = (icon) => ((D.iconSet && D.iconSet.web) || 'icons/{icon}.webp').replace('{icon}', icon);
  const modIcon = (m, cls = '') => (m && m.icon
    ? `<img class="ico ${cls}" src="${iconSrc(m.icon)}" alt="" loading="lazy" title="${esc(m.name)} · ${esc(m.icon)}" style="border-color:${D.rarityColors[m.rarity] || 'var(--line)'}">`
    : `<span class="ico ph ${cls}" style="border-color:${(m && D.rarityColors[m.rarity]) || 'var(--line)'}"></span>`);
  ...
  R.mods = () => {
    ...
    const rows = list.map((m) => { const wr = m.weapon || {}; return `<tr data-act="m-json" data-id="${m.id}" style="cursor:pointer"><td>${modIcon(m)}</td><td>${rarPill(m.rarity)}</td><td>${typePill(m.type)}</td><td><b>${esc(m.name)}</b><div class="mono muted">${esc(m.id)}</div></td><td>${effectChips(m)}</td><td>${esc(m.desc)}${m.charge ? `<div class="small" style="color:var(--warn)">Заряд: ${esc(m.charge)}</div>` : ''}${m.cooldown ? `<div class="small muted">КД ${m.cooldown} с</div>` : ''}</td><td class="small">${wr.requires?.length ? `<div>треб: ${wr.requires.map(tn).join(wr.requiresAny ? ' / ' : ' + ')}</div>` : ''}${Object.entries(wr.bonus || {}).map(([t, v]) => `<span class="tag" style="color:var(--good)">${esc(tn(t))} +${v}</span>`).join('')}${Object.entries(wr.penalty || {}).map(([t, v]) => `<span class="tag" style="color:var(--bad)">${esc(tn(t))} ${v}</span>`).join('')}</td><td class="num">${m.maxStacks || 1}</td><td class="num">${m.power || 0}</td></tr>`; }).join('');
    return `<div class="row"><h1 style="margin:0">Модификаторы</h1><span style="flex:1"></span><button class="btn primary" data-act="m-add">+ Модификатор</button></div>
      ...
      <div class="tablewrap"><table><thead><tr><th></th><th>Редк.</th><th>Тип</th><th>Название</th><th style="min-width:260px">Эффекты (триггер ▸ доставка ▸ нагрузка; ⇠ = наследует статусы)</th><th style="min-width:240px">Описание</th><th>Оружие</th><th class="num">Стак</th><th class="num">Сила</th></tr></thead><tbody>${rows}</tbody></table></div>
  ...
  R.synergy = () => {
    ...
    const pairList = pairs.map((p) => `<div class="pair" data-act="to-builder-pair" data-a="${modA.id}" data-b="${p.mod.id}" data-module="${p.isModule ? p.mod.slot : ''}" title="открыть пару в конструкторе">
      <div style="min-width:200px;display:flex;align-items:center;gap:6px">${p.isModule ? '' : modIcon(p.mod, 'sm')}<span>${rarPill(p.mod.rarity)} <b>${esc(p.mod.name)}</b>${p.isModule ? ` <span class="tag wt">модуль · ${esc(D.moduleSlots[p.mod.slot] || p.mod.slot)}</span>` : ''}${p.aff.ok ? '' : ' <span class="err" title="несовместим с текущим оружием">✕</span>'}</span></div>
  ...
  R.builder = () => {
    ...
    const libHtml = lib.map((m) => { const a = affinity(tags, m); const on = inBuild(m.id); let g = ''; if (m.type !== lastT) { lastT = m.type; g = `<div class="lib-g">${typeName[m.type]}</div>`; }
      return g + `<div class="lib-i ${a.ok ? '' : 'dis'} ${on ? 'on' : ''}" data-act="lib-toggle" data-id="${m.id}" title="${esc(a.ok ? (a.reasons.join(', ') || 'нейтрально к оружию') : a.reasons[0])}">${modIcon(m, 'sm')}<div style="flex:1;min-width:0"><div class="nm">${esc(m.name)} ${on ? '<span class="chk">✓</span>' : ''}</div><div class="ef">${effectChips(m) || `<span class="muted small">${esc(m.desc)}</span>`}</div></div><span class="aff" style="opacity:${a.ok ? 1 : 0.5}">${a.ok ? (a.score === 1 ? '' : a.score) : '✕'}</span></div>`; }).join('');

    const entry = (x, kind) => { const m = M(x.id); if (!m) return ''; const a = affinity(tags, m); return `<div class="be" style="border-left-color:${D.rarityColors[m.rarity]}">${modIcon(m, 'sm')}<div style="flex:1;min-width:0"><div class="nm">${esc(m.name)} <span class="muted small">${a.ok ? (a.score !== 1 ? 'aff ' + a.score : '') : '<span class="err">несовместим</span>'}</span></div><div class="ef">${effectChips(m)}</div></div>${kind === 'passive' && (m.maxStacks || 1) > 1 ? `<input type="number" min="1" max="${m.maxStacks}" value="${x.stacks || 1}" data-stack="${m.id}" title="стаки" style="width:46px">` : ''}<button class="btn sm ghost" data-act="rm" data-id="${m.id}">✕</button></div>`; };
    const buildHtml = `
      <div class="card">
  ...
  function draftPanel(build, res) {
    ...
    const offerCards = offer.length ? offer.map((x) => `<div class="offer" style="border-color:${D.rarityColors[x.mod.rarity]}">
        <div class="row" style="gap:8px;align-items:flex-start">${modIcon(x.mod, 'lg')}<div style="flex:1;min-width:0">
        ...
        <div class="ef" style="margin:3px 0">${effectChips(x.mod)}</div><div class="small muted">${esc(x.mod.desc)}</div></div></div>
    ...
    const top = cands.list.slice(0, 12).map((x) => `<div class="pair" style="cursor:default"><div style="min-width:180px;display:flex;align-items:center;gap:6px">${modIcon(x.mod, 'sm')}<span>${rarPill(x.mod.rarity)} <b>${esc(x.mod.name)}</b></span></div><div class="mono small" style="width:56px">${(x.p * 100).toFixed(1)}%</div><div class="small muted" style="flex:1">${esc(previewText(x)) || '—'}</div></div>`).join('');
  ...
  document.addEventListener('click', (e) => {
    ...
    const H = {
      ...
      'm-add': () => jsonEditor('Новый модификатор', { id: 'new_mod', name: 'Новый мод', type: 'passive', rarity: 'common', icon: 'skill_001', tags: [], maxStacks: 1, power: 3, desc: '', stats: {}, effects: [{ trigger: 'on_hit', payload: 'burn' }], weapon: {} }, (v) => { if (M(v.id)) return 'id занят'; D.mods.push(v); }),
